...
m <- leaflet(width = 1200, height = 1100, padding = 0, options = leafletOptions(minZoom = 5, maxZoom = 7)) %>% 
  addGlPolylines(base_lyr, color = 'black', weight = 0.5)

for (i in 1:length(shrub_lyrs)){
  m <- m %>% 
    addGlPoints(data = shrub_lyrs[[i]], fillColor = shrub_pal[i], group = names(shrub_lyrs)[i] %>% str_replace_all('_', ' '), fillOpacity = 1.0, radius = 5)
}

m <- m %>% 
  addLayersControl(overlayGroups = names(shrub_lyrs) %>% str_replace_all('_', ' ')) %>% 
